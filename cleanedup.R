...
# USER SETTINGS
# =========================================================
data_dir <- "~/Downloads/raw_data"
sample_conditions <- c(rep("Treatment", 7), rep("Control", 8))
...
# Remove Ensembl version suffixes like ".12"
strip_version <- function(x) {
  sub("\\.[0-9]+$", "", x)
}

# Find Salmon quant files and name them by sample
get_salmon_files <- function(dir, pattern = "_quant\\.sf$") {
  files <- list.files(dir, pattern = pattern, full.names = TRUE)
  if (length(files) == 0) {
    stop("No Salmon quant files found. Check dir/pattern.")
  }
...
# Pull transcript-to-gene mappings from Ensembl
get_tx_maps <- function(mart) {
  ...
  list(
    tx2gene_symbol = tx2gene_symbol,
    tx2gene_ens = tx2gene_ens,
    id_to_symbol = id_to_symbol
  )
}

# Simple PCA QC plot from raw counts
plot_pca_qc <- function(counts, metadata,
                        title = "Bulk RNA-seq PCA (QC Check)",
                        subtitle = NULL) {
...
# Plot one deconvolved cell type at a time
...
# Merge two matrices by union of genes, filling missing genes with zeros
merge_gene_mats_union <- function(mat_a, mat_b) {
  all_genes <- union(rownames(mat_a), rownames(mat_b))

  fill_missing_with_zeros <- function(mat, all_genes) {
  ...
  cbind(
    fill_missing_with_zeros(mat_a, all_genes),
    fill_missing_with_zeros(mat_b, all_genes)
  )
}

# Build a named stat vector for GSEA, restoring names if needed
make_named_stat <- function(res_obj, dds_obj = NULL) {
  stats <- res_obj$stat
  stats <- stats[!is.na(stats)]

  # DESeq2 results sometimes lose names; recover them if needed
  if (is.null(names(stats)) || all(is.na(names(stats))) || all(names(stats) == "")) {
    if (!is.null(rownames(res_obj)) && length(rownames(res_obj)) >= length(res_obj$stat)) {
      result_names <- rownames(res_obj)[!is.na(res_obj$stat)]
      names(stats) <- result_names
    } else if (!is.null(dds_obj)) {
      if (length(rownames(dds_obj)) == length(res_obj$stat)) {
        result_names <- rownames(dds_obj)[!is.na(res_obj$stat)]
        names(stats) <- result_names
      }
    }
  }
...
# Convert named stats to ENTREZ IDs for GSEA
map_to_entrez <- function(stat_named, fromType) {
  ...
  # Keep the strongest statistic if multiple IDs map to same ENTREZ gene
...
# Run GO BP GSEA safely, trying ENSEMBL first then SYMBOL if needed
...
# Add gene symbols to GEO differential results
attach_symbols_from_gse <- function(gset, ad_results) {
  ...
  if (is.na(sym_col)) {
    sym_col <- grep("Gene", colnames(feature_data), ignore.case = TRUE)[1]
  }
...
# Find rows where any metadata column contains a keyword
...
# =========================================================
# PHASE 1: SALMON IMPORT (SYMBOL LEVEL) + PCA QC
# =========================================================
files <- get_salmon_files(data_dir)
maps <- get_tx_maps(mart)

txi_symbol <- tximport(
  files,
  type = "salmon",
  tx2gene = maps$tx2gene_symbol,
...
preview_table <- data.frame(
  ...
  Assigned_Condition = sample_conditions
)
print(preview_table)

metadata <- make_metadata(colnames(raw_counts), sample_conditions)
print(
  plot_pca_qc(
    raw_counts,
    metadata,
    subtitle = "Prior Manipulation"
  )
)
...
sce_nonmyeloid <- TabulaMurisSenisFACS(tissues = "Brain_Non-Myeloid")

sce_myeloid_raw <- if (is.list(sce_myeloid)) sce_myeloid[[1]] else sce_myeloid
sce_nonmyeloid_raw <- if (is.list(sce_nonmyeloid)) sce_nonmyeloid[[1]] else sce_nonmyeloid

sce_myeloid_hip <- sce_myeloid_raw[, grepl("Hippocampus", sce_myeloid_raw$subtissue, ignore.case = TRUE)]
sce_nonmyeloid_hip <- sce_nonmyeloid_raw[, grepl("Hippocampus", sce_nonmyeloid_raw$subtissue, ignore.case = TRUE)]

print(paste("Myeloid Cells:", ncol(sce_myeloid_hip)))
print(paste("Non-Myeloid Cells:", ncol(sce_nonmyeloid_hip)))

counts_myeloid <- as.matrix(counts(sce_myeloid_hip))
counts_nonmyeloid <- as.matrix(counts(sce_nonmyeloid_hip))
reference_counts <- merge_gene_mats_union(counts_myeloid, counts_nonmyeloid)

meta_cols <- c("subtissue", "cell_ontology_class", "mouse.id")
meta_myeloid <- colData(sce_myeloid_hip)[, meta_cols, drop = FALSE]
meta_nonmyeloid <- colData(sce_nonmyeloid_hip)[, meta_cols, drop = FALSE]
reference_meta <- rbind(meta_myeloid, meta_nonmyeloid)

valid_cells <- !is.na(reference_meta$cell_ontology_class)
reference_counts <- reference_counts[, valid_cells, drop = FALSE]
reference_meta <- reference_meta[valid_cells, , drop = FALSE]

ref_eset <- ExpressionSet(
  assayData = reference_counts,
  phenoData = AnnotatedDataFrame(as.data.frame(reference_meta))
)

overlap <- intersect(rownames(raw_counts), rownames(ref_eset))
print(paste("Overlapping genes found:", length(overlap)))
if (length(overlap) < 2000) {
  stop("Low overlap: check ID types (symbol vs ensembl).")
}

bulk_eset <- ExpressionSet(
  assayData = raw_counts[overlap, , drop = FALSE]
)
...
grid.arrange(
  plot_cell_type(plot_props, "Oligodendrocytes", condition_colors),
  plot_cell_type(plot_props, "Microglia", condition_colors),
  plot_cell_type(plot_props, "Astrocytes", condition_colors),
  ncol = 3
)


# =========================================================
# PHASE 5: DESEQ2 (ENSEMBL LEVEL) + SYMBOL ANNOTATION
# =========================================================
txi_ens <- tximport(
  files,
  type = "salmon",
  tx2gene = maps$tx2gene_ens,
  ignoreTxVersion = TRUE
)

metadata_ens <- make_metadata(colnames(txi_ens$counts), sample_conditions)
...
# Save DEG table with attached gene symbols
...
res_final$Significant <- ifelse(
  !is.na(res_final$padj) &
    res_final$padj < 0.05 &
    abs(res_final$log2FoldChange) > 0.5,
  "Sig",
  "NS"
)

res_final$Label <- ifelse(
  res_final$Symbol %in% hero_genes & res_final$Significant == "Sig",
  res_final$Symbol,
  NA
)
...
p_volcano <- ggplot(res_final, aes(log2FoldChange, -log10(padj))) +
  ...
  labs(
    title = "Volcano Plot: Young CSF Effect",
...
vsd_mat <- assay(vsd)
rownames(vsd_mat) <- strip_version(rownames(vsd_mat))
mat_top <- vsd_mat[top_genes, , drop = FALSE]

symbol_lookup <- maps$id_to_symbol$Symbol[
  match(rownames(mat_top), maps$id_to_symbol$Ensembl_ID)
]

rownames(mat_top) <- ifelse(
  is.na(symbol_lookup),
  rownames(mat_top),
  symbol_lookup
)
...
vsd_symbols <- maps$id_to_symbol$Symbol[
  match(rownames(mat_vsd), maps$id_to_symbol$Ensembl_ID)
]

mat_sym <- mat_vsd
rownames(mat_sym) <- vsd_symbols
...
if (length(group_wt) == 0 || length(group_ad) == 0) {
  stop("Could not identify groups from metadata.")
}

ex <- exprs(gset)
if (max(ex, na.rm = TRUE) > 50) {
  ex <- log2(ex + 1)
}
...
print(paste(
  "Fisher enrichment p:",
  format.pval(fisher.test(mat_f, alternative = "greater")$p.value)
))
...
up_genes <- my_res_clean$Symbol[
  my_res_clean$My_LogFC > 0.25 & my_res_clean$My_Padj < 0.05
]
...
if (length(up_genes) > 0) {
  ...
  if (nrow(tf_sig) > 0) {
    ...
    p_tf <- ggplot(
      top_tfs,
      aes(reorder(Term, -log10(Adjusted.P.value)), -log10(Adjusted.P.value))
    ) +
      geom_bar(stat = "identity", alpha = 0.75) +
      coord_flip() +
      labs(
        title = "Top Upstream Regulators (TRRUST)",
    ...
    p_chea <- ggplot(
      ...
      coord_flip() +
      labs(
        title = "Top Upstream Regulators (ChEA)",
